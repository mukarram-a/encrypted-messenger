...
Mukarram's Note-taking Program -- Allows users to create, save, and upload encrypted notes and posts.
- Features an online messaging functionality using public server
- Graphical interface for easy use
- Encrypts all messages, posts, user input, and files for security.
...
# v0.4
# 
# The following module provides a graphical user interface shell for the DSP journaling program. Includes code from Mark Baldwin.
...
class Body(tk.Frame):
    ...
    def set_posts(self, posts:list):
        ...
        self._posts = posts
        for index, i in enumerate(self._posts):
            self._insert_post_tree(index, i)
...
class Footer(tk.Frame):
    ...
    def online_click(self):
        if self._online_callback is not None:
            self._online_callback(bool(self.is_online.get()))
...
class MainApp(tk.Frame):
    ...
    def publish(self, post:Post):
        '''
        Publish a post from the GUI to the online DSU server using encryption

        '''
        ds_client.send(self._current_profile.dsuserver, 3021, self._current_profile.username, self._current_profile.password, post.entry, self._current_profile.bio, self._current_profile.public_key, self._current_profile)
